...
def apply_filter(image, ftype):
    ...
    if ftype == "red_tint":
        ...
        decrease = np.array([20, 10, 0], dtype=np.uint8)
...
def main():
    ...
    if not cap.isOpened():
        ...
        return
    ftype = "original"
    print("Keys: r = Red, g = Green, b = Blue, s = Sobel, c = Canny, t = Cartoon, x = sepia, z = sketch, w = warm, o = cool, q = Quit")
    while True:
        ret, frame = cap.read()
        if not ret:
